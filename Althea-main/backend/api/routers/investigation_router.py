...
from core.security import get_current_user, get_tenant_id, require_any_permission, require_permissions
...
@router.post("/cases")
def create_case(request: Request, payload: CreateCaseRequest, tenant_id: str = Depends(get_tenant_id)):
...
@router.get("/actor")
def get_actor(request: Request, tenant_id: str = Depends(get_tenant_id)):
    return {"actor": request.app.state.case_service.get_actor(tenant_id, _user_scope(request))}


@router.put("/actor")
def set_actor(request: Request, payload: SetActorRequest, tenant_id: str = Depends(get_tenant_id)):
    request.app.state.case_service.set_actor(tenant_id, _user_scope(request), payload.actor)
    return {"actor": payload.actor}


@router.put("/cases/{case_id}")
def update_case(case_id: str, request: Request, payload: UpdateCaseRequest, tenant_id: str = Depends(get_tenant_id)):
    run_info = request.app.state.pipeline_service.get_run_info(tenant_id, _user_scope(request))
