...
@router.get("/api/runs")
def list_runs(request: Request, tenant_id: str = Depends(get_tenant_id)):
    return {"runs": request.app.state.pipeline_service.list_runs(tenant_id)}


@router.get("/api/alerts/{alert_id}")
def get_alert(alert_id: str, request: Request, tenant_id: str = Depends(get_tenant_id)):
...
def get_alert_explain(alert_id: str, request: Request, run_id: Optional[str] = None, tenant_id: str = Depends(get_tenant_id)):
...
def get_queue_metrics(request: Request, tenant_id: str = Depends(get_tenant_id)):
    run_id = _active_run_id(request, tenant_id)
