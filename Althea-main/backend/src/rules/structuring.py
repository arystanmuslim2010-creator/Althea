...
def run_rule(df: pd.DataFrame, cfg) -> pd.DataFrame:
    ...
    for user, group in df.groupby("user_id", sort=False):
        group_sorted = group.sort_values("ts", kind="mergesort")
        grp = group_sorted.set_index("ts")
        direction_out = grp["direction"].astype(str).str.lower() == "out"
        amount_numeric = pd.to_numeric(grp["amount"], errors="coerce")
        qualifies = (
            direction_out
            & (amount_numeric >= low)
            & (amount_numeric < high)
        ).astype(int)
        rolling_counts = qualifies.rolling(f"{window_days}D").sum().fillna(0)
        counts = rolling_counts.values
        hits = counts >= min_count
        scores = np.minimum(1.0, counts / max(min_count * 2, 1))

        df.loc[group_sorted.index, hit_col] = hits.astype(int)
        df.loc[group_sorted.index, score_col] = scores
        evidence_list = []
        result_list = []
        ...
        df.loc[group_sorted.index, evidence_col] = evidence_list
        df.loc[group_sorted.index, result_col] = result_list
